...
```javascript
{
    "coin": "PIRATE.json",                              // the file in 'coins' folder that defines current used coin
    "address": "RQf6QUVqtcv6D63Tf8njZDphNG9f9tfyHm",    // the transparent address you want reward to go to
    "rewardRecipients": {                               // an array of optional address(es) and percentage(s) you want portion of reward to go to
        "RV5EeB2iVDa2Nki7Y8Sk5YzL8gSczpTNSv": 16        // Must either be real or empty cannot be like "": 16
    },
...
```javascript
{
    ...
    "algorithm": "equihash",    // equihash inless someone makes a verushash AND `ac_private` coin
