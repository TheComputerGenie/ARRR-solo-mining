...
var pool = module.exports = function pool(options, authorizeFn) {
    ...
    function SetupPeer() {
        ...
        _this.peer.on('connected', function () {
            ...
            emitErrorLog('p2p connection failed - node rejected you');
    ...
    // I contemplated removing this for solo, but can be used to create tax/partner accounts
    function SetupRecipients() {
        ...
        for (var r in options.rewardRecipients) {
            ...
            var rObj = { percent: percent, address: r };
    ...
    function DetectCoinData(finishedCallback) {
        ...
        _this.daemon.batchCmd(batchRpcCalls, function (error, results) {
            ...
            options.initStats = {
                ...
                difficulty: rpcResults.getinfo.difficulty * algo.multiplier,
        ...
        _this.stratumServer.on('started', function () {
            ...
            client.on('difficultyChanged', function (diff) {
                ...
                if (typeof (options.ports[client.socket.localPort]) !== 'undefined') {
                    this.sendDifficulty(_this.jobManager.currentJob.difficulty * algo.multiplier);
                } else {
                    this.sendDifficulty(_this.jobManager.currentJob.difficulty * algo.multiplier);
                }
    ...
    this.processBlockNotify = function (blockHash, sourceTrigger) {
        // TODO: using currentJob writes 1 block ahead on vlidation
        if (!process.env.forkId || process.env.forkId === '0') { emitLog('Block notification via ' + sourceTrigger + ' -->> Block: ' + JSON.stringify(_this.jobManager.currentJob.rpcData.height)); }
