...
var BlockTemplate = module.exports = function BlockTemplate(jobId, rpcData, recipients, poolAddress) {
    var submits = [];
    this.rpcData = rpcData;
    this.jobId = jobId;
    this.target = bignum(rpcData.target, 16);
    this.difficulty = parseFloat((algo.diff / this.target.toNumber()).toFixed(9));
    ...
    this.serializeBlock = function (header, soln) {
        ...
        buf = new Buffer.concat([
            ...
            soln,
            varInt,
            new Buffer(this.generationTransaction, 'hex')
        ]);
